...
router.get('/metrics', authenticate, async (req, res) => {
  try {
    ...
    const statusOpenClause = appendCondition(scope.clause, 'a.status IN (?, ?, ?, ?)');
    const deliveredClause = appendCondition(scope.clause, 'a.status IN (?, ?, ?, ?)');
    ...
    const [[approved]] = await db.query(
      'SELECT COUNT(*) AS count FROM evaluations WHERE verdict = ?',
      ['Aprobado']
    );
    const [[unreadAlerts]] = await db.query(
      'SELECT COUNT(*) AS count FROM alerts WHERE recipient_role IN (?) AND is_read = FALSE',
      [alertRoles]
    );
...
router.get('/pipeline', authenticate, async (req, res) => {
  try {
    ...
    for (const status of statuses) {
      const clause = appendCondition(scope.clause, 'a.status = ?');
